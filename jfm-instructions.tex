% Modifications from Marie-Jean Thoraval (mjthoraval@gmail.com)
% Version: 8th February 2018
% Run in TeXnicCenter with option LaTeX => DVI => PDF
%
...
\usepackage{patches} % Use jfm_jabbrv.bst in \bibliographystyle instead of jfm.bst
...
\begin{itemize}
	...
	BibTeX style file \verb|jfm_jabbrv.bst| in accordance with the instructions of that package.
	See \citet{Thoraval2012}.
	% https://github.com/compholio/jabbrv.
	% http://www.compholio.com/latex/jabbrv/
	% https://tex.stackexchange.com/questions/33441/how-to-abbreviate-journal-name-in-citation
	\item Replace the first name of authors in the References with their first name initials. See \citet{Thoraval2012}.
	...
	\item Replace with \textit{et al.} in the references list if more than ten authors, in accordance with the recommendations in these guidelines.
	...
	% Compare with original unsrt.bst: http://ftp.cs.stanford.edu/tex/bibtex/unsrt.bst
	% https://www.diffchecker.com or https://text-compare.com/
	...
	\item Added hyperlinks in the references section. It chooses the DOI first, and then the URL if there is no DOI.
	No link is added if neither of them is provided in the bibtex file.
	A complete bibtex entry can be generated from Mendeley for example \citep{Thoraval2012}.
	\item Added a patch of natbib to show the hyperlink only on the year in the text, as in the published version of the articles.
	% https://tex.stackexchange.com/questions/23227/how-to-hyperlink-only-the-year-part-when-using-natbib-and-hyperref
	\item Updated natbib.sty to \verb|[2010/09/13 8.31b (PWD, AO)]| to use the patch above.
\end{itemize}
...
%\bibliographystyle{jfm} % Original bibliograhy style
\bibliographystyle{jfm_jabbrv} % Bibliography style to use abbreviated journal titles
